...
source( paste0(params$path, "/sampleSheet.r")  )
...
#1- load pdata
pdata<-readSampleSheet(params$pdata, samples=params$samples, barcode=params$barcode, groups=params$groups, sep=params$separator)
if(file.exists(params$badsamples)){
  pdata<-removeBadSamples(pdata,params$bad)
}
nsamp<-nrow(pdata)
```
...
message(paste0("reading idat : " , platform ) )
nbprobes1<-nrow(betas)
message(paste0("reading idat : " , nbprobes1, " probes" ) )
pvalues<-minfi::detectionP(RGset)
```

```{r checkgender, include=FALSE}
#7-check gender and age
...
jpeg(paste0(params$out, "/redchannel.jpg"), width=800, height=800)
boxplot(log2(getRed(RGset)+1), main="Red channel", col = "red", las=2, cex.axi=0.8, labels=pdata$samples)
dev.off()

jpeg(paste0(params$out, "/greenchannel.jpg"), width=800, height=800)
...
for (group in groups) {
	...
	densityBeanPlot(RGset, sampGroups = pdata[,group])
	dev.off()
...
#11- Houseman
pdata<-houseman(RGset, pdata, platform, params$out)
```
...
if(params$normalize=="SWAN")    {   MSet<-preprocessSWAN(RGset, MSet )                        }
...
message(paste0(filteredNAprobes, " probes removed because percentage of NA values > ", params$nalimit))
...
for (group in groups) {
	...
	if (nrow(pdata) < 200 ){
		mdsPlot(betas,numPositions=1000,sampGroups=pdata[,group],
					legendPos = "bottomleft",sampNames=pdata$samples)
	}else{
		mdsPlot(betas,numPositions=1000,sampGroups=pdata[,group])
	}
	dev.off()
...
```{r PCA, include=FALSE}
#20- PCA
pca<-makepca( betas, pdata, params$out, colnames(pdata), nPC=10 )
...
#22-violin plot
violin_plot(pdata=pdata, betas=betas, group=groups[1], samp="samples", platform=platform, datadir=datadir, out=params$out, regions="")
```
...
write.table(pca$pvalue, file=paste0(params$out, "/pvalue_0001.txt"), row.names=T, sep="\t")
write.table(pca$qvalue, file=paste0(params$out, "/qvalue_0001.txt"), row.names=T, sep="\t")

regions=""
save(betas, pdata, samples, groups, platform, genome, regions, mval, regression, file=paste0(params$out, "/meth.rdata") )
```
...
### [ Red Channel ](`r params$out`/redchannel.jpg)

```{r, echo=FALSE}
  knitr::include_graphics(paste0(params$out,"/redchannel.jpg"))
```

### [ Green Channel ](`r params$out`/greenchannel.jpg)

```{r, echo=FALSE, results='asis'}
  knitr::include_graphics(paste0(params$out,"/greenchannel.jpg"))
```
...
```{r, echo=FALSE, eval = file.exists(paste0(params$out,"/violin1.jpg")), results ='asis' }
  cat( "### [ case control ](",params$out,"/violin1.jpg)\n\n", sep="")
  knitr::include_graphics(paste0(params$out,"/violin1.jpg"))
```

```{r, echo=FALSE, eval = file.exists(paste0(params$out,"/violin2.jpg")), results ='asis' }
  cat( "### [ CPG regions ](",params$out,"/violin2.jpg)\n\n", sep="")
  knitr::include_graphics(paste0(params$out,"/violin2.jpg"))
```

```{r, echo=FALSE, eval = file.exists(paste0(params$out,"/violin3.jpg")), results ='asis' }
