...
mkPlotDMR<-function(dmr){
	...
	print(dmrname)
	img<-paste0(out, "/", process_id, "/", dmrname, ".dmrplot.jpg")
	tryCatch({
		jpeg(paste(img ,sep="/"), width=800, height=800)
...
dmrs<-tmpl(paste(readLines(paste0(path, "/templates/dmrplot.html.tpl")), collapse="\n"))
