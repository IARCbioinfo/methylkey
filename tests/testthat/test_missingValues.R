test_that("cpg_na_excl identifies probes with many NAs", {
  ...
  excluded <- cpg_na_excl(betas, nalimit = 0.2)
...
test_that("cpg_na_excl handles matrix with no NAs", {
  betas <- matrix(runif(100), nrow = 20, ncol = 5)
  rownames(betas) <- paste0("cg", sprintf("%07d", 1:20))
...
test_that("cpg_na_excl respects nalimit parameter", {
