...
cat(paste0("method:",params$regression,"\n"),  append=TRUE, file=log)
cat(paste0("genome:",params$genome,"\n"),      append=TRUE, file=log)
```
...
if(platform=="RRBS"){
  regression$table<-annotateSites(regression$table,as.data.frame(regions)[,c(1,2,3,6,7,5)])
}else{
...
dmrcoutput<-dmrcate(foo,C=2, pcutoff=params$pval)
...
toptable<-myannotation[ annot.symbol!="NA", -c("annot.gene_id","annot.seqnames","annot.strand","annot.start","annot.end","annot.width","annot.id","annot.tx_id","annot.type"),]
toptable<-unique(toptable)
if ( params$dmrcate ){
  toptable[, annot.symbol:=paste(annot.symbol, collapse=","), by="dmr"]
}else{
  toptable[, annot.symbol:=paste(annot.symbol, collapse=","), by="probe"]
}
toptable<-unique(toptable)

if(params$dmrcate){
  toptable<-toptable[ HMFDR < params$fdr ][ order(HMFDR,meandiff,maxdiff) ]
  nbdmrs=nrow(toptable)
  message( paste0("you get ", nbdmrs, " significant DMRS with", params$regression))
  cat(paste0("nbdmrs:",nbdmrs, "\n"), append=TRUE, file=log)
...
if (hsize>1) {
  	...
  	aheatmap(betas[as.character(cpgs),], annCol=pdata[,groups], labRow=labrow)
  	dev.off()
}
```
...
#16 save objects
save(betas, mval, pdata, groups, samples, design, platform, genome, regions, toptable, model, info, file=paste0(rpath, "/meth.rdata") )
``` 
