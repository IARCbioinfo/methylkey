...
if ( max==0 & length(cpgs)==0) { stop("no dmps to plot") }
...
txdb<-getTxdb(genome)
load(paste0(path, "/data/cpgIslands.", genome, ".rda"))
...
cometCpg<-function(){
	...
	tryCatch({
			...
			dev.off()
			img<-paste0(out, "/", process_id, "/", ref.cpg, ".comet.jpg")
...
comet<-tmpl(paste(readLines(paste0(path, "/templates/comet.html.tpl")), collapse="\n"))
