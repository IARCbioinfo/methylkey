...
if(filetype=="gzfile"){
	...
	betas<-fread(params$idat, header=T, sep=",", fill=TRUE)
}
regions<-betas[,1:5, with=F]
pdata<-pdata[ pdata$samples %in% colnames(betas),]
samples<-pdata$samples
#betas<-betas[, colnames(betas) %in% pdata$samples, with=F]
betas<-betas[,pdata$samples, with=F]
message( paste0( length(pdata$samples), " samples in pdata, ", ncol(betas), " found in betas" ) )
...
```{r PCA, include=FALSE}
#10- PCA
pca<-makepca( betas, pdata, params$out, colnames(pdata) )
pvalue<-pca$pvalue
qvalue<-pca$qvalue
