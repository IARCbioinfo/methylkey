...
dmpsFileName <- paste0("dmps_", model_name, "_", index, ".tsv")
readr::write_tsv(dmps,file = fs::path(output_dir,dmpsFileName))
nbdmps <- dmps |> dplyr::filter(adj.P.Val < 0.05) |> nrow()
```
...
dmps |> dplyr::filter(adj.P.Val < 0.05) |>
  dplyr::arrange(adj.P.Val) |> head(100) |>
  DT::datatable(
    extensions = 'Buttons',
    options = list(
      dom = 'Blfrtip',
...
gr_list <- gr_list[lengths(gr_list) > 0]
Peaks <- lapply(gr_list, function(gr) {
  ChIPseeker::annotatePeak(TxDb = txdb, peak = gr, verbose = FALSE)
})
ChIPseeker::plotAnnoBar(Peaks)
...
# 1. Ta liste de gènes
my_genes <- get_genes(mrs, index) |> head(100)
