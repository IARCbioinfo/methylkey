...
sankeyPlot_ <- function(df, v_space = "auto", term_size = 3, gene_size = 1.5, palette = NULL) {
  ...
  if (is.null(palette)) {
    palette <- c(
      set_names(
        viridis::viridis(n_distinct(df$Term)),
  ...
  sankey_plot <- ggplot(
    ...
    geom_text(aes(label = node, cex = stage),
      position = pos,
      stat = "sankeynode",
      hjust = 1
    ) +
...
dotPlot_ <- function(df) {
  ...
  dotplot <- df |>
    group_by(Term, P.value, yy, status, gene_count) |>
    mutate(Odds.Ratio = max(Odds.Ratio)) |>
    ggplot(aes(x = Odds.Ratio, y = yy, size = gene_count, color = logP)) +
    geom_point(alpha = 0.8) +
    geom_rect(aes(
      xmin = min(Odds.Ratio) - 0.05 * max(Odds.Ratio),
      ymin = 0,
      xmax = max(Odds.Ratio) + 0.05 * max(Odds.Ratio),
      ymax = max(yy) + 0.2 * max(yy)
    ),
    color = "black",
    fill = NA,
    linewidth = 0.1
...
sankeyDotPlot <- function(
  ...
  df <- df |>
    mutate(
      logP = -log10(P.value),
      Term = fct_reorder(Term, gene_count)
    ) |>
...
get_enrichr_url <- function(genes, description = "Ma liste de gènes") {
  ...
  if (httr::status_code(response) == 200) {
    ...
    return(paste0("https://maayanlab.cloud/Enrichr/enrich?userListId=", user_list_id))
  } else {
