...
sampleSheet <- normalizePath(params$sampleSheet) %>% readr::read_delim() %>% dplyr::select(-c(Date,Sample_Well,Plate_ID)) %>%
  tidyr::separate(Basename,into=c("Sentrix_ID","Sentrix_Position"),remove = FALSE) 
#%>% filter(Sentrix_ID==206402350147 | Sentrix_ID==206425820155)
...
saveRDS(meth,file="EBV.minfi.meth.rds")
```
...
saveRDS(meth,file="EBV.minfi.methM.rds")
```
...
```{r model2}
mrs<-methyldiff2(model="~ebv+sentrix_id",
           case ="YES",
           control="NO",
           se=methM,
           method="ls"
