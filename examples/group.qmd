...
for(grp in params$groups){
  cat("\n\n#### ", grp, "\n\n")
  grp=methylkey::colData(methM) %>% pull(tolower(grp))
  p <- my_scatterPlot(mvals, grp, samples(methM), "npg", 1, TRUE, TRUE)
  print(p)
}
```
...
```{r, cache=TRUE}
pdata<-methylkey::colData(methM) %>% dplyr::select(samples,group,sentrix_position,sentrix_id)
...
for(grp in tolower(params$groups) ){
  cat("\n\n#### ", grp, "\n\n")
  factoextra::fviz_pca_ind(pca, axes=c(1,2), habillage=methylkey::colData(methM) %>% dplyr::pull(tolower(grp)), addEllipses=TRUE ) %>% print()
}
```
