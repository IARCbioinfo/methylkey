...
readmeth<-function(pdata, idat , samples, groups, out, normalize="funnorm", cell="None"){
	...
	boxplot(log2(RGset@assays$data$Red+1), main="methylated (Red channel)", col = "red", las=2, cex.axi=0.8)
	dev.off()

	jpeg(paste0(out, "/unmethylated1.jpg"), width=800, height=800)
	boxplot(log(RGset@assays$data$Green+1), main="unmethylated (Green channel)", col = "green", las=2, cex.axi=0.8)
	dev.off()
	...
	for (group in groups) {
		...
		if (nrow(pdata) < 200 ){
			mdsPlot(betas,numPositions=1000,sampGroups=pdata[,group], legendPos = "bottomleft",sampNames=samples)
		}else{
...
houseman<-function(RGset=RGset, pdata=pdata, out){
	...
	wbc<-estimateCellCounts(RGset, returnAll=T, meanPlot=T, sex=pdata$predictedSex )
	dev.off()
